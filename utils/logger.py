...
class Logger(object):
    ...
    def __init__(self, log_dir, name=None, remove_all_log=False):
        ...
        if name is not None:
            try:
                if remove_all_log and os.path.exists(os.path.join(log_dir, name)):
                    shutil.rmtree(os.path.join(log_dir, name))
    ...
    def model_param_histo_summary(self, model, step, postfix=''):
